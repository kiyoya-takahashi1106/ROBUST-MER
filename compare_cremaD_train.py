...
from model.compare_cremaD_train_model import No_Linear_Model
...
def train(args):
    ...
    log_dir = os.path.join("runs", "compare_cremaD", f"{args.dataset_name}_seed{args.seed}_dropout{args.dropout_rate}")
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if (accuracy >= best_acc):
            best_acc = accuracy
            os.makedirs("saved_models/compare_cremaD/", exist_ok=True)
            torch.save(model.state_dict(), f"saved_models/compare_cremaD/{args.dataset_name}_classNum{args.class_num}_{date}_epoch{epoch}_{accuracy:.4f}_seed{args.seed}_dropout{args.dropout_rate}.pth")
