...
class Model(nn.Module):
    ...
    def one_forward(self, modality, x, encoder, attn_mask, encoder_layer_norm, linear, shared_layer_norm, dropout):
        with torch.no_grad():
            if (modality == "audio"): 
                encoder_output = self.encoder(input_values=x, attention_mask=attn_mask, return_dict=True)
            elif (modality == "video"):
    ...
    def forward(self, audio_x, video_x, audio_attn_mask, video_attn_mask):
        ...
        # fusion_f = torch.stack((audio_divided_f, video_divided_f), dim=0)
        # fusion_f = self.transformer_encoder(fusion_f)
        # fusion_f = torch.cat((fusion_f[0], fusion_f[1]), dim=1)
